...
public class MacroExecutor implements Runnable {
    ...
    private void executeBaritoneCommandInternal(String command) {
        try {
            ...
            // 执行命令（通过发送聊天消息的方式，避免映射问题）
            // 注意：Baritone 会监听聊天消息中的 # 命令
            Minecraft mc = Minecraft.getInstance();
            if (mc != null && mc.player != null && mc.getConnection() != null) {
                // 发送 #命令 到聊天框，Baritone 会自动处理
                String chatCommand = "#" + fullCommand;
                mc.getConnection().sendChat(chatCommand);
                LOGGER.info("[宏执行] 已通过聊天消息发送 Baritone 命令: {}", chatCommand);
            } else {
                LOGGER.warn("[宏执行] 无法发送聊天消息，玩家或连接未初始化");
            }
