...
public class DeathHandler {
    ...
    public static void initialize() {
        ...
        // 使用平台抽象注册客户端 Tick 事件
        ClientTickHandler.registerClientTick(DeathHandler::onClientTick);
        initialized = true;
        LOGGER.info("[死亡处理] 死亡处理器已初始化");
    }
    ...
    private static void stopAllBaritoneTasks(Minecraft mc) {
        try {
            ...
            try {
                ...
                if (baritone != null && mc.getConnection() != null) {
                    // 通过聊天消息发送 stop 命令，避免映射问题
                    mc.getConnection().sendChat("#stop");
