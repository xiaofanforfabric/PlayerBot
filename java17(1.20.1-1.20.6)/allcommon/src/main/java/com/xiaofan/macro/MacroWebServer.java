...
public class MacroWebServer {
    ...
    public static void initialize() {
        ...
        try {
            ...
            getLogger().info("[宏管理] Web 服务器已启动，访问 http://localhost:{} 管理宏", HTTP_PORT);
            
        } catch (java.net.BindException e) {
            getLogger().error("[宏管理] 端口 " + HTTP_PORT + " 已被占用");
            isRunning = false;
        } catch (IOException e) {
            getLogger().error("[宏管理] 启动失败: " + e.getMessage(), e);
            isRunning = false;
        }
    }
    ...
    public static void stop() {
        if (httpServer != null && isRunning) {
            try {
                httpServer.stop(0);
                getLogger().info("[宏管理] Web 服务器已停止");
            } catch (Exception e) {
                getLogger().error("[宏管理] 停止失败: " + e.getMessage());
            }
            httpServer = null;
            isRunning = false;
        }
    ...
    private static class ListMacrosHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            try {
                ...
                getLogger().error("[宏管理] 列出宏文件时出错", e);
