...
public class AutoSleepController {
    ...
    public static void initialize() {
        ...
        // 注册 HOME 键监听
        KeyInputHandler.registerKeyPress(GLFW.GLFW_KEY_HOME, AutoSleepController::toggle);
        
        // 使用平台抽象注册客户端 Tick 事件
        ClientTickHandler.registerClientTick(AutoSleepController::onClientTick);
        
        initialized = true;
        LOGGER.info("[自动睡觉] 自动睡觉控制器已初始化，按 HOME 键切换开关");
    }
    ...
    private static void toggle() {
        ...
        if (mc.player != null) {
            ...
            mc.player.sendSystemMessage(Component.literal("§7[自动睡觉] " + status));
        }
